...
def main():
    ...
    pd_exerpt, data_deriv_0, moldelR1_deriv_0 = fitcountry(places, nbperiodes, decalage, surplus, verbose, plot, 'piecewise')
    ...
    d_exerpt, data_deriv_1, moldelR1_deriv_1 = fitcountry(places, nbperiodes, decalage, surplus, verbose, plot, 'All')


    # Plot all in one figure
    for indexcountry in range(len(listplaces)):
        country  = listplaces[indexcountry]
...
def PlotAll(data_deriv_0, model_deriv_0, data_deriv_1, model_deriv_1, title, filename):
    ...
    ax.plot(time, data_deriv_0[1:],  label=r'$\frac{\partial R^1(n)}{\partial n}$ - piecewise',  color='green', alpha=1.0, lw=0.5, marker='x')
    ax.plot(time, model_deriv_0[1:], label=r'$\frac{\partial R^1(t)}{\partial t}$ - piecewise',  color='blue',  alpha=0.4, lw=1.3)
    # ax.plot(time, data_deriv_1[1:],  label=r'$\frac{\partial R^1(n)}{\partial n}$ - All', color='blue',   alpha=1.0, lw=0.5, marker='x')
    ax.plot(time, model_deriv_1[1:], label=r'$\frac{\partial R^1(t)}{\partial t}$ - All', color='green',   alpha=0.7, lw=1.3)
...
def fitcountry(places, nbperiodes, decalage, surplus, verbose, plot, ch):

    pd_exerpt, data_deriv, moldelR1_deriv = fit([places, nbperiodes, decalage, surplus, verbose, plot])
