...
def fit(sysargv):
	...
	for indexplace, place in enumerate(listplaces):
		...
		if plot==True:
			repertoire = getRepertoire(UKF_filt, './figures/SEIR1R2D_UKFilt/'+placefull+'/sexe_'+str(sexe)+'_delay_'+str(decalage), './figures/SEIR1R2D/'+placefull+'/sexe_'+str(sexe)+'_delay_'+str(decalage))
